...
- `api/enterprise_suite.py`, `api/quick_order.py`, `api/compliance.py`, `api/llm_support.py`, `cli/`, `openapi/`, `observability/`: Workflow, ACL, kargo/iade/bildirim, hızlı sipariş, sertifika/müfredat readiness, LLM görev planlama, CLI, API sözleşmesi ve izlenebilirlik katmanı.
- `api/static/assets/3d/`, `docs/visual-assets-tr.md`, `schemas/visual_asset_schema.json`: 3D SVG demo görselleri, Vercel yayın hattı görseli ve görsel varlık şeması.
- `vercel.json`, `api/vercel_app.py`, `scripts/deploy_vercel.sh`, `docs/vercel-deploy-tr.md`: Vercel serverless yayın hazırlığı, kontrol scripti ve final deploy runbook'u.

## Otomotiv Yedek Parça Ekosistemi

Platform; parça kataloğu, OEM/araç uyumluluk araması, çok lokasyonlu stok, sipariş yönetimi ve barkodlu el terminali görev kuyruğunun yanında ERP, EDI, CRM, PIM, finans, lojistik, BI, güvenlik, audit, SaaS ve on-prem dağıtım taslakları için genişletilmiş tailor-made B2B çekirdeği sunar. API başladıktan sonra panelden canlı özeti görmek için `http://127.0.0.1:8000`, Swagger için `http://127.0.0.1:8000/docs` adresini açın. Ayrıntılı otomotiv akışı için `docs/automotive-ecosystem-tr.md`, uçtan uca B2B mimarisi için `docs/enterprise-b2b-ecosystem-tr.md`, dev depo yol haritası için `docs/mega-repository-plan-tr.md`, sertifika/müfredat go-live matrisi için `docs/compliance-certification-roadmap-tr.md`, LLM desteği için `docs/llm-support-tr.md`, 3D görseller için `docs/visual-assets-tr.md`, Vercel final yayını için `docs/vercel-deploy-tr.md`, profesyonel başlangıç-final akışı için `docs/pro-launch-runbook-tr.md` dosyasına bakın.
